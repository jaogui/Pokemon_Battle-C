...
// Funcao para verificar se o jogador ja existe e obter seu Pokemon
int verificarJogadorExistente(Jogador *jogador) {
    Jogador jogadorExistente;
    FILE *arquivo = fopen("jogador.bin", "rb");
    if (arquivo == NULL) {
        return 0; // Retorna 0 se não houver arquivo
    }

    while (fread(&jogadorExistente, sizeof(Jogador), 1, arquivo)) {
        // Imprime o nome e Pokémon lidos para depuração
        printf("Lendo jogador: %s, Pokemon: %s\n", jogadorExistente.nome, jogadorExistente.pokemonEscolhido);
        if (strcmp(jogadorExistente.nome, jogador->nome) == 0) {
            // Copia o Pokémon escolhido se o jogador já existir
            strcpy(jogador->pokemonEscolhido, jogadorExistente.pokemonEscolhido);
            fclose(arquivo);
            return 1; // Retorna 1 se o jogador já existir
        }
    }

    fclose(arquivo);
    return 0; // Retorna 0 se o jogador não existir
}
...
int main() {
    ...
    if (verificarJogadorExistente(&jogador)) {
        printf(Verde "Bem-vindo de volta, %s! Seu Pokemon %s estava esperando por voce!\n", jogador.nome, jogador.pokemonEscolhido);
    } else {
